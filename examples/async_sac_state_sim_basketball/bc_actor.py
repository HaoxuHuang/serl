...
flags.DEFINE_integer("save_demo_period", 0, "Period to save demos.")
...
def actor(agent: SACAgent, data_store, env, sampling_rng):
    ...
    for step in tqdm.tqdm(range(FLAGS.actor_steps), dynamic_ncols=True):
        ...
        with timer.context("sample_actions"):
            ...
            correct_actions = agent.sample_actions(
                observations=jax.device_put(obs),
                argmax=True,
            )
            actions = np.asarray(jax.device_get(actions))
            correct_actions = np.asarray(jax.device_get(correct_actions))
        ...
        if FLAGS.save_demo_period and step % FLAGS.save_demo_period == 0:
            print_green(f"Saving demo at step {step}")
            with open(FLAGS.save_demo_path, "wb") as f:
                pickle.dump(data_store, f)
            print_green(f"Saved demo to {FLAGS.save_demo_path}")
...
def main(_):
    ...
    assert FLAGS.load_checkpoint is not None
    assert FLAGS.save_demo_path is not None
